 ...
 * 클래스의 타입도 abstract class가 되어야한다.
 * abstract class 는 인스턴스로 만들 수가 없다. (new Seller() ====> X)
 */
public abstract class Seller {
	...
	//클래스 상수에 값을 할당하는 방법 - static block
	static {
	...
	public void insertMoney(Customer customer, String productName, int orderCount) { //Customer 클래스를 가져옴.  {
	//	this.money += this.product.getPrice(); // 기계의 돈에 product price 1600을 가져옴.
	//	customer.pay(this.product.getPrice()); // customer 클래스의 pay를 통해 1600을 차감.
		// this.productArray를 반복하면서 Product 인스턴스의 제품명 확인//Product 인스턴스 제품명과 productName이 같으면,
		// 해당 제품의 가격으로 자판기의 돈을 증가시키고 고객의 돈을 감소시킨다.
	...
	public void pressButton(Customer customer, String productName, int orderCount) {
		...
		// 같으면 해당 제품의 수량을 체크하고 (0보다 작은지) 작다면, 메소드를 종료시킨다.
		// 그렇지 않다면 해당 제품의 수량을 하나 감소시키고 고객에게 해당제품을 전달한다.
		for ( Product product : this.productArray ) {
			if (product.equals(productName)) {
